<div>
    <form wire:submit.prevent="save" class="grid gap-4 gap-y-2 text-sm grid-cols-8">
        ...
        <div class="mt-4 col-span-8 md:col-span-5">
            <x-input wire:model.defer="participante.name" label="Nome completo do(a) participante" />
        </div>
        <div class="mt-4 col-span-8 md:col-span-3">
            <x-input wire:model.defer="participante.cracha" label="Nome para o crachá" />
        </div>

        <div class="col-span-8 md:col-span-3">
            <x-tall.cpf wire:model.lazy="participante.cpf" label="CPF" />
        </div>
        <div class="col-span-8 md:col-span-2">
            <x-input wire:model.defer="participante.rg" label="RG" />
        </div>
        <div class="col-span-8 md:col-span-3">
        ...
        <div class="col-span-8 md:col-span-3">
            <x-select wire:model.defer="participante.tipo_inscricao_id" label="Tipo de Inscrição">
                @if ($instituicao)
        ...
        @if ($hotel = $model->hotel)
            <!-- criar campo hotel na tabela inscrições e gravar junto com os dados do usuário/evento-->
            <!--  comentei pq o marcio dispensou nesse primeiro momento, mas para o próximo precisaemos incluir o hotel.   -->
            <div class="col-span-8 md:col-span-5 flex items-center">
                <div class="mt-6">
                    <x-toggle lg wire:model.defer="participante.hotel"
                        label="{{ sprintf('Desejo reservar hotel juntamente com essa inscrição. R$ %s', form_read($hotel->valor_single)) }}" />
                </div>
            </div>
        @endif
                    ...
                    <div>
                        @if (data_get($participante, 'id'))
                            <x-button type="button" class="w-full" primary squared spinner="save" size="lg"
                                label="Inscrever mais participante" />
                        @endif
                    </div>
