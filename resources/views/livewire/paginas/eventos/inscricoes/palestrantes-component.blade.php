    ...
    <div x-data="{ sidebarOpen: false, darkMode: false }" :class="{ 'dark': darkMode }">
        <div class="flex bg-gray-100 dark:bg-gray-800 font-roboto">
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                @include('livewire.paginas.eventos.inscricoes.includes.header', ['title'=>'PALESTRANTES PARA'])
                    ...
                    <div class="container mx-auto px-6 py-8">
                        ...
                        <section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-4 py-6">
                            <div class="w-full text-center pb-8">
                                ...
                                <p class="text-gray-400 font-normal text-base">Confira nosso time</p>
                            </div>
                            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                                @if ($palestras = $model->palestras)
                                    @foreach ($palestras as $palestra)
                                        <div class="bg-white rounded-lg p-6">
                                            <div class="flex items-center space-x-6 mb-4">
                                                <img class="h-28 w-28 object-cover object-center rounded-full"
                                                    src="{{ $palestra->user->profile_photo_url }}"
                                                    alt="photo">
                                                <div>
                                                    <p class="text-xl text-gray-700 font-normal mb-1">{{ $palestra->user->name }}</p>
                                                    <p class="text-base text-blue-600 font-normal">{{ $palestra->name }}</p>
                                                </div>
                                            </div>
                                            <div>
                                                <p class="text-gray-400 leading-loose font-normal text-base">Biografia:
                                                   @if ($descripition = $palestra->description)
                                                   {{ $descripition->preview }}
                                                   @endif 
                                                </p>
                                            </div>
                                        </div>
