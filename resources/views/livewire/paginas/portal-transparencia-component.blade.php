        ...
        <div class="rounded-md flex items-center">
            <div class="flex flex-col text-gray-800 text-center sm:text-left w-full">
                <h1 class="text-4xl font-bold mb-4">
                    Apeprev - Portal da Transparência
                </h1>
                <section class="flex flex-col w-full">
                    <!-- BEGIN: breadcrums v1 -->
                    <x-breadcrums> Portal da Transparência</x-breadcrums>
                    <!-- END: breadcrums v1 -->
                </section>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                  @if($detalhe = $model->detalhe)  {{ $model->detalhe->name }} | @endif {{ $model->name }}
                </h2>
                <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                    <div class="mt-2 flex items-center text-sm text-gray-500">
                        ...
                        {{ __('Data referência') }}: {{ date_carbom_format($model->reference_date)->format('d/m/Y') }}
                        @if ($model->data_validade)
                            | {{ __('Data validade') }}:
                            {{ date_carbom_format($model->data_validade)->format('d/m/Y') }}
                        @endif
                    </div>
                </div>
            </div>
            <div class="mt-5 flex lg:mt-0 lg:ml-4">
                <span class="sm:ml-3">
                    <a href="{{ \Storage::url($model->file->url) }}" target="_blank"
                        ...
                        <span class="ml-2">{{ __('Baixar') }}</span>
                    </a>
                </span>
            </div>
        </div>
