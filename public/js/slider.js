...
window.Slider = function (options) {
  ...
  this.option = Object.assign(this.option, options); // Set _this so we can access it from anywhere
  ...
  var slides = slider.querySelectorAll(_this.option.slideSelector); // Add dots to dotContainer if present
  ...
  if (_this.option.touchwipe && 'ontouchstart' in window) {
    ...
    slider.addEventListener('touchmove', function (evt) {
      ...
      xDown = null;
    }, false); // Hide arrow keys on touch devices
